...
void android_cliprdr_complete_text_send(androidContext* afc, BOOL accepted)
{
	const UINT64 requestId = afc->clipboardRequestId;
	afc->clipboardRequestId = 0;
	afc->clipboardRequestSerial = 0;
	if (requestId)
		freerdp_callback("OnClipboardSendResult", "(JJZ)V",
		                 (jlong)afc->common.context.instance, (jlong)requestId,
...
UINT android_cliprdr_send_client_format_list(CliprdrClientContext* cliprdr)
{
	...
	/* Responses have no request ID. Match them in channel order, including
	 * ordinary clipboard sync and the initial MonitorReady format list. */
	if (afc->clipboardRequestId && afc->clipboardRequestSerial)
		android_cliprdr_complete_text_send(afc, FALSE);
	afc->clipboardListSerial++;
	if (afc->clipboardRequestId)
		afc->clipboardRequestSerial = afc->clipboardListSerial;
                                               ...
                                               const CLIPRDR_FORMAT_LIST* formatList)
{
	...
	/* A remote copy supersedes the text we were about to paste. */
	android_cliprdr_complete_text_send(afc, FALSE);
                                            ...
                                            const CLIPRDR_FORMAT_LIST_RESPONSE* formatListResponse)
{
	...
	if (!afc)
		return ERROR_INVALID_PARAMETER;
	if (afc->clipboardResponseSerial < afc->clipboardListSerial)
		afc->clipboardResponseSerial++;
	if (afc->clipboardRequestId &&
	    afc->clipboardRequestSerial == afc->clipboardResponseSerial)
		android_cliprdr_complete_text_send(
		    afc, (formatListResponse->common.msgFlags & CB_RESPONSE_OK) != 0);
...
BOOL android_cliprdr_init(androidContext* afc, CliprdrClientContext* cliprdr)
{
	...
	afc->cliprdr = cliprdr;
	afc->clipboardListSerial = 0;
	afc->clipboardResponseSerial = 0;
	afc->clipboardRequestId = 0;
	afc->clipboardRequestSerial = 0;
